...
export default function CreateProductPage() {
  ...
  const [mainMeta, setMainMeta] = useState<ImageMeta>({
    angle: 0,
    isTryOn: false,
  });

  const [galleryMeta, setGalleryMeta] = useState<ImageMeta[]>([]);
  ...
  return (
    ...
    "
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div>
            <label className="block mb-2 text-sm text-slate-300">Length</label>
          ...
          <div>
            <label className="block mb-2 text-sm text-slate-300">Density</label>
          ...
          <div>
            <label className="block mb-2 text-sm text-slate-300">Texture</label>
          ...
          <div>
            <label className="block mb-2 text-sm text-slate-300">Color</label>
        ...
        <div className="border border-white/10 rounded-2xl p-6 mt-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-white">Included Items</h2>
            ...
            "
              >
                {ANGLE_OPTIONS.map((angle) => (
                  <option key={angle} value={angle} className="bg-slate-900">
                    {angle}&deg;
                    {angle === 0 ? " (front)" : ""}
                  </option>
                ))}
        ...
        <div>
          ...
          <p className="mb-3 text-xs text-slate-500">
            Tick &ldquo;Use for try-on&rdquo; on the transparent wig cut-outs
            and set the angle each one was shot at. Virtual try-on loads only
            these, and cross-fades between the two nearest the viewer&rsquo;s
            head angle &mdash; a front (0&deg;) plus both profiles (45&deg; and
            315&deg;) is the useful minimum.
          </p>
          ...
          {galleryPreviews.length > 0 && (
            ...
            "
                  ...
                  <label className="flex items-center gap-2 text-xs text-slate-300">
                    <input
                      type="checkbox"
                      checked={galleryMeta[index]?.isTryOn ?? false}
                      onChange={(e) =>
                        setGalleryMeta((meta) =>
                          meta.map((item, i) =>
                            i === index
